...
function createMenuItem(myname, myprice, mycategory){
  const newItem = {};
  newItem.name = myname;
  newItem.price = myprice;
  newItem.category = mycategory;
  return newItem;
}

 console.log(createMenuItem('Chicken and Waffles',12,'Brunch'));
...
createMenuItem('licorice pizza',6,'anything');
createMenuItem('poo pie',12,'desert');
createMenuItem('cotton candy sundae',8,'desert');
...
const burger = {
  ...
  discount: function (customer) {
    if (customer === 'teacher' || customer === 'student') {
      var newPrice = burger.price * 0.75;
    } else if (customer === 'public') {
      var newPrice = burger.price * 0.90;
    }
    return(newPrice);
  }
}
...
function addReview(arr,name,rating,feedback){
  ...
  newRev.feedback = feedback;
  arr.push(newRev);
  return arr;
}

console.log(addReview(reviews,'Sydney',4,'Alright'));
...
function getReviewByIndex(arr,ind) {
  var stmnt = `${arr[ind].name} gave the restaurant a ${arr[ind].rating} star review, and their feedback was: ${arr[ind].feedback}`
  return stmnt;
}
...
function getLastReview(arr) {
  var lastrev = `${arr[arr.length-1].name} gave the restaurant a ${arr[arr.length-1].rating} star review, and their feedback was: ${arr[arr.length-1].feedback}`
  return lastrev;
} 
